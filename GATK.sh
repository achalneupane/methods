...
mkdir -p dbSNP SAM BAM sortedBAM SNPs/persample SNPs/MERGED SNPs/MERGED/INDELs SNPs/MERGED/SNPs SNPs/MERGED/both
...
MERGED_INDELs="${SNPs}MERGED/INDELs/"
MERGED_SNPs="${SNPs}MERGED/SNPs/"
both="${SNPs}MERGED/both/"
...
java -Xmx10g -jar ${GATK}GenomeAnalysisTK.jar -T SelectVariants -R ${GATK}dog_chr5.fa -V ${MERGED}raw_variants.vcf -selectType SNP -o ${MERGED_SNPs}raw_SNP.vcf 
java -Xmx10g -jar ${GATK}GenomeAnalysisTK.jar -T SelectVariants -R ${GATK}dog_chr5.fa -V ${MERGED}raw_variants.vcf -selectType INDEL -o ${MERGED_INDELs}raw_INDEL.vcf 


# Distribution of variants
cp ${MERGED}INDELs/* ${both}
cp ${MERGED}SNPs/* ${both}
cd "$both"
...
java -Xmx10g -jar ${GATK}GenomeAnalysisTK.jar -T VariantFiltration -R ${GATK}dog_chr5.fa -V ${MERGED_SNPs}raw_SNP.vcf \
--filterExpression "QD < 2.0 || FS > 60.0 || MQ < 40.0" \
--filterName "snp_filter" \
-o ${MERGED_SNPs}filtered_SNP.vcf

java -Xmx10g -jar ${GATK}GenomeAnalysisTK.jar -T VariantFiltration -R ${GATK}dog_chr5.fa -V ${MERGED_INDELs}raw_INDEL.vcf \
--filterExpression "QD < 2.0 || FS > 200.0" \
--filterName "indel_filter" \
-o ${MERGED_INDELs}filtered_INDEL.vcf
